...
public class NumberCardFrame extends JFrame {
  ...
  private final List<String> displayedFields = Arrays.asList("number",
    "countryIndex",
    "dataHumans",
    "fullNumber",
    "countryText",
  ...
  public NumberCardFrame(PhoneNumber phoneNumber, JFrame previousFrame) {
    SwingUtilities.invokeLater(() -> {
      ...
      List<String> attributeNames = Arrays.stream(phoneNumber.getClass().getDeclaredFields())
        .map(Field::getName)
        .toList();//Собирает в лист названия полей
      ...
      for (String attributeName : attributeNames) {
        try {
          addLabelAndTextToContentPane(contentPane, attributeName, getFieldValue(phoneNumber, attributeName));
  ...
  private @NotNull JButton getSaveButton(PhoneNumber phoneNumber) {
  ...
  public void addLabelAndTextToContentPane(Container container, String attributeName, String attributeValue) {
